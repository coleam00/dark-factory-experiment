...
async def keyword_search(
    ...
    Args:
        ...
        allowed_video_ids: Optional video-level ACL filter — if provided,
            only chunks whose video_id is in this list are returned. None
            disables the filter (whole-library behavior).
...
async def set_conversation_video_scope(conv_id: str, user_id: str, video_scope: list[str]) -> bool:
    """Set the video scope on an unscoped conversation. Returns False if the
    conversation does not belong to the user or its scope is already set.
    """
    async with _acquire() as conn:
        result = await conn.execute(
            """
            UPDATE conversations
            SET video_scope = $1, updated_at = $2
            WHERE id = $3 AND user_id = $4 AND video_scope IS NULL
            """,
            video_scope,
            _now(),
            conv_id,
