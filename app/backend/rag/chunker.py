...
def chunk_video_timestamped(segments: list[TimestampedSegment]) -> tuple[list[dict], bool]:
    ...
    for segment in segments:
        ...
        try:
            ...
            # Only applies to positive-duration segments: zero/negative-duration
            # segments (e.g. the final segment from _parse_segments, where
            # end == start) keep the original boundary on every sub-chunk.
            if len(sub_chunks) > 1:
