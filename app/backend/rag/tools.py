...
async def execute_search_hybrid(
    ...
    try:
        ...
        chunks = await retrieve_hybrid(
            query, embedding, top_k=top_k, is_member=is_member, allowed_video_ids=allowed_video_ids
        )
    except Exception as exc:
...
async def execute_get_video_transcript(
    ...
    if allowed_video_ids is not None and video_id not in allowed_video_ids:
        return {
            "ok": False,
            "error": f"video_id {video_id!r} is not in this conversation's video scope.",
        }
