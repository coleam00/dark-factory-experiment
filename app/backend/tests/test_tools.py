...
async def test_execute_search_hybrid_happy_path(monkeypatch) -> None:
    async def fake_retrieve(_q, _emb, top_k=5, is_member=False, scope_video_ids=None):
        assert top_k == 10
        return _FAKE_CHUNKS
...
async def test_execute_search_hybrid_respects_per_video_cap(monkeypatch) -> None:
    ...
    async def fake_retrieve(_q, _emb, top_k=10, is_member=False, scope_video_ids=None):
        return many_chunks
